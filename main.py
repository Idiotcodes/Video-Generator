...
import os
from helper import detect_object, get_inference, generate_image, generate_audio, generate_video
...
def text_to_video():
  if request.method == 'POST':
    ...
    text_data = request.form[
        'textInput']  # Assuming the text input has id "textInput"
...
def image_to_video():
  if request.method == 'POST':
    ...
    if file and allowed_file(file.filename):
      ...
      keywords_file_path = os.path.join(app.static_folder, 'output',
                                        'gen_keyword.txt')
      with open(keywords_file_path, 'w') as f:
        for obj in object_detected:  # Assuming object_detected is a list of objects
          f.write(obj)

      description_file_path = os.path.join(app.static_folder, 'output',
                                           'gen_desc.txt')
